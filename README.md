# MC Macaco Bot - Automated Video Generation Platform
...
```
bots/
├── mc_macaco_bot.py         # Main video generation bot
...
### MC Macaco Bot (Video Generation)

```bash
# Run directly
python bots/mc_macaco_bot.py

# Run in background
nohup python bots/mc_macaco_bot.py > mc_macaco.log 2>&1 &
```
...
```bash
# View logs
tail -f mc_macaco.log
tail -f link_downloader.log

# Stop bots
pkill -f mc_macaco_bot.py
